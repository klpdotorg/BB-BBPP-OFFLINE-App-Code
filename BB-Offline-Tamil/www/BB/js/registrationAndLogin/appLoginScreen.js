...
Game.appLoginScreen.prototype = {
	...
	localdatasuccess: function (result) {
		...
		if (result.rows.length > 0) {
			...
			// No local users — this build is language-locked (window.languageSelected is set
			// in index.js), so skip the language picker and register directly, same as the
			// offline branch of appLoginScreenbbpp.js.
			_this.state.start('appLoginEditScreen', true, false, window.languageSelected, window.app_Mode);
		}
		// if (result.rows.length > 0) {
